...
begin
  ...
  Check(FiscalPrinter.SetPOSID('Êàññà: 7', 'ÊÀÑÑÈÐ: Êðàâöîâ Â.Â.'));
  AddLine('Print test receipt');
  PrintTestReceipt;
  // Cashier2
  AddLine(Separator);
  AddLine('SetPOSID('''', ''Cashier2'')');
  Check(FiscalPrinter.SetPOSID('Êàññà: 7', 'ÊÀÑÑÈÐ: Èâàíîâ È. È.'));
  AddLine('Print test receipt');
  PrintTestReceipt;
  // Some text
  AddLine(Separator);
...
begin
  ...
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:       8199'));
  // EndFiscalReceipt
  AddLine('EndFiscalReceipt');
  Check(FiscalPrinter.EndFiscalReceipt(True));
  // Test completed
...
begin
  ...
  FiscalPrinter.AdditionalTrailer := AdditionalTrailer;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Áóëêà âêóñíàÿ è ñâåæàÿ', 123.45, 123, 1, 0, ''));
  Check(FiscalPrinter.PrintRecItem('Áóëêà âêóñíàÿ è ñâåæàÿ', 123.45, 123, 2, 0, ''));
  Check(FiscalPrinter.PrintRecItem('Áóëêà âêóñíàÿ è ñâåæàÿ', 123.45, 123, 3, 0, ''));
  Check(FiscalPrinter.PrintRecItem('Áóëêà âêóñíàÿ è ñâåæàÿ', 123.45, 123, 4, 0, ''));
...
begin
  ...
  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Áóëêà âêóñíàÿ è ñâåæàÿ', 123.45, 123, 0, 0, ''));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(FPTR_AT_AMOUNT_DISCOUNT, 'Ñêèäêà 14.50', 14.50, 0));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(FPTR_AT_AMOUNT_SURCHARGE, 'Íàäáàâêà 20.50', 20.50, 0));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Áóëêà âêóñíàÿ è ñâåæàÿ', 123.45, 123, 0, 0, ''));

  AddLine('PrintRecItemVoid');
  Check(FiscalPrinter.PrintRecItemVoid('Êàðòîôåëü ìîëîäîé', 123.45, 123, 0, 0, ''));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Êîíü â ÿáëîêàõ', 123.45, 123, 0, 0, ''));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(FPTR_AT_AMOUNT_DISCOUNT, 'Ñêèäêà 12.34', 12.34, 0));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(FPTR_AT_AMOUNT_SURCHARGE, 'Íàäáàâêà 23.45', 23.45, 0));

  AddLine('PrintRecSubtotalAdjustment');
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(FPTR_AT_AMOUNT_DISCOUNT, 'Ñêèäêà 10%', 10.00));
...
begin
  ...
  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Êîñòþì               2000013553010',799,0,1,0,''));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Êîñòþì               2000013553010',799,0,1,0,''));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Íîñêè                2000013552693',139,0,1,0,''));
...
begin
  ...
  Check(FiscalPrinter.PrintRecItem('Item1', 141.40, 1000, 1, 141.40, 'èâ'));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'Discount1', 42.42));
  Check(FiscalPrinter.PrintRecItem('Item2', 141.40, 1000, 1, 141.40, 'èâ'));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'Discount2', 42.42));
  Check(FiscalPrinter.PrintRecItem('Item3', 146.60, 1000, 1, 146.60, 'èâ'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Discount3', 36.65, 1));
  Check(FiscalPrinter.PrintRecItem('Item4', 146.60, 1000, 1, 146.60, 'èâ'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Discount4', 36.65, 1));
  Check(FiscalPrinter.PrintRecItem('Item5', 22.20, 1000, 2, 22.20, 'èâ'));
  Check(FiscalPrinter.PrintRecItem('Item6', 106.20, 1000, 1, 106.20, 'èâ'));
  Check(FiscalPrinter.PrintRecItem('Item7', 11.10, 1000, 2, 11.10, 'èâ'));
...
begin
  ...
  Check(FiscalPrinter.PrintRecItem('ÊÎËÁÀÑÊÈ ÍÞÐÍÁÅÐÃÑÊÈ', 212.91, 852, 2, 249.9, 'êã'));
  Check(FiscalPrinter.PrintRecVoidItem('ÊÎËÁÀÑÊÈ ÍÞÐÍÁÅÐÃÑÊÈ', 249.9, 476, 0, 0, 2));
...
begin
  ...
  Check(FiscalPrinter.PrintRecItem('ÌÀÍÄÀÐÈÍÛ ÌÀÐÎÊÊÎ', 71.6, 1075, 1, 66.6, 'ì '));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêè ïîêóï', 7.16, 1));
  Check(FiscalPrinter.PrintRecItem('ÃÎÐÁÓ?À Ï/Ê Á/Ã', 184.93, 740, 2, 249.9, 'êã'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêè ïîêóï', 36.99, 2));
  Check(FiscalPrinter.PrintRecItem('ÂÎÄÊÀ ÐßÁ×ÈÊ 40% 0.5', 147, 1000, 1, 147, 'øò'));
  Check(FiscalPrinter.PrintRecSubtotal(359.38));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'ÂÎÄÊÀ ÐßÁ×ÈÊ 40% 0.5', 44.1));
  Check(FiscalPrinter.PrintRecItem('ÑÅËÜÄÜ Â ÊÐÀÑÍÎÌ ÂÈÍ', 53.2, 1000, 2, 53.2, 'øò'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêè ïîêóï', 13.3, 2));
...
begin
  ...
  FiscalPrinter.AdditionalTrailer := AdditionalTrailer;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('ÏÈÂÎ ÎÊÑÊÎÅ ÆÈÂÎÅ 4', 69.10, 1000, 1, 69.10, 'øò'));
  Check(FiscalPrinter.PrintRecItem('ÌÎÉÂÀ ÊÐÓÏÍÀß Ñ/Ì ÐÎ', 36.93, 740, 2, 49.90, 'êã'));
  Check(FiscalPrinter.PrintRecItem('ÌÎËÎÊÎ ÒÎÏË. 4% 1/45', 25.30, 1000, 2, 25.30, 'øò'));
  Check(FiscalPrinter.PrintRecItem('ÌÀÍÄÀÐÈÍÛ ÌÀÐÎÊÊÎ', 41.93, 700, 1, 59.90, 'ì '));
  Check(FiscalPrinter.PrintRecItem('ÑÛÐÊÈ ÍÅÆÍÛÅ ÈÇÞÌ 1/', 19.90, 1000, 2, 19.90, 'øò'));
  Check(FiscalPrinter.PrintRecItem('ÏÐÎÄÓÊÒ ÒÂÎÐÎÆÍÛÉ Ñ', 22.50, 1000, 2, 22.50, 'øò'));
  Check(FiscalPrinter.PrintRecItem('ÁÀÍÀÍÛ', 12.03, 325, 1, 37, 'ì '));
...
begin
  ...
  FiscalPrinter.AdditionalTrailer := AdditionalTrailer;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Item1', 10, 1000, 1, 10, 'øò'));
...
begin
  ...
  FiscalPrinter.AdditionalTrailer := AdditionalTrailer;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('ÌÀÍÄÀÐÈÍÛ', 98.34, 1775, 1, 55.4, 'ì '));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêè ïîêóï', 9.83, 1));
  Check(FiscalPrinter.PrintRecItem('ÒÎÌÀÒÛ', 52.43, 750, 2, 69.9, 'ì '));
  Check(FiscalPrinter.PrintRecItem('ÐÓËÅÒ ÀÐÎÌÀÒÍÛÉ', 209.52, 900, 2, 232.8,'êã'));
  Check(FiscalPrinter.PrintRecItem('ÇÅÔÈÐ ÂÀÍÈËÜÍÛÉ', 33.96, 238, 1, 142.7,'êã'));
  Check(FiscalPrinter.PrintRecSubtotal(384.42));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'ÇÅÔÈÐ ÂÀÍÈËÜÍÛÉ', 10.19));
  Check(FiscalPrinter.PrintRecItem('ÑÛÐ ÎÐÈ×ÅÒÒÈ ÊÎÏ×ÅÍÛ', 69.9, 1000, 2, 69.9, 'øò'));
  Check(FiscalPrinter.PrintRecItem('ÐÈÑ ÊÐÀÑÍÎÄÀÐÑÊÈÉ 1/', 42.4, 1000, 2, 42.4, 'øò'));
  Check(FiscalPrinter.PrintRecSubtotal(486.53));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'ÑÊÈÄÊÀ 7 %', 26.2));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'ÊÓÏÎÍ', 460.33));
...
begin
  ...
  Check(FiscalPrinter.ResetPrinter);
  // Set header
  Check(FiscalPrinter.SetHeaderLine(1, getImageCommand('Logo.bmp') + 'ÎÎÎ "ÃÈÏÅÐÃËÎÁÓÑ"', False));
  Check(FiscalPrinter.SetHeaderLine(2, 'ã.Âëaäèìèð Ñóçäaëüñêèé ïðîñïåêò 28', False));
  Check(FiscalPrinter.SetHeaderLine(3, 'ò(4922)37-68-66', False));
  ...
  Check(FiscalPrinter.printRecItem('ÍÅÑÊÀÔÅ ÊËÀÑÑÈÊ 120Ã', 108.90, 1000, 1, 108.90, ''));
  ...
  Check(FiscalPrinter.printRecMessage('*****     Ñïañèáî ça ïîêóïêó!      *****'));
  Check(FiscalPrinter.printRecMessage('****************************************'));
  Check(FiscalPrinter.printRecMessage('"Ãëîáóñ" ã.Âëàäèìèð ïðèãëàøàåò íà ðàáîòó'));
  Check(FiscalPrinter.printRecMessage(' Ïðîäàâåö-êàññèð(Íàïèòêè)); Ïîâàð;'));
  Check(FiscalPrinter.printRecMessage(' Ïîìîøíèê ïîâàðà(ðåñòîðàí));'));
  Check(FiscalPrinter.printRecMessage(' Ïðîäàâåö-êîíñóëüòàíò(îáóâü)); Âîäèòåëü'));
  Check(FiscalPrinter.printRecMessage(' ïîãðóç÷èêà; Ïðîäàâåö(îòäåë ïðîäàæ)'));
  Check(FiscalPrinter.printRecMessage('          Òåë. (4922) 37-68-54'));
  Check(FiscalPrinter.printRecMessage('        Íàéäè ðàáîòó óæå ñåãîäíÿ!'));
  Check(FiscalPrinter.endFiscalReceipt(True));
...
begin
  ...
  for i := 1 to 10 do
  begin
    Check(FiscalPrinter.PrintRecItem('Ïðîäàæà ' + IntToStr(i), 1, 1000, 0, 0, ''));
  end;
  Check(FiscalPrinter.PrintRecTotal(100, 100, '0'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
  TickCount := Integer(GetTickCount) - TickCount;
  AddLine('Âðåìÿ âûïîëíåíèÿ: ' + IntToStr(TickCount));
end;
...
begin
  ...
  for i := 1 to 20 do
  begin
    Check(FiscalPrinter.PrintRecItem('Ïðîäàæà ' + IntToStr(i), 0, 1000, 0, 0, ''));
  end;
...
begin
  ...
  begin
    ...
    Check(FiscalPrinter.PrintRecItem(Format('Ïðîäàæà %d', [i]), 0, 1000, 0, 0, ''));
  end;
...
ÀÎ "ÐÍ-Ìîñêâà"
MJ019 Âåëüÿìèíîâî
ÌÎ, ã. Äîìîäåäîâî, ìêð. Áàðûáèíî,
69 êì Êàøèðñêîãî ø., ñòð.1
 
 
ÊÊÌ 00013801              ÈÍÍ ???????????? #6998
18.02.16 14:13
ÏÐÎÄÀÆÀ                                    ¹3930
Coca-cola ïë.áóò. 1ë
01                                        ?95.00
...
begin
  ...
  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Coca-cola ïë.áóò. 1ë', 95.00, 1000, 0, 0, ''));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(1,'3% Loyalty 000302992000000040', 2.85, 0));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Áàáàåâñê/ïîìàä-ñëèâ', 37.90, 1000, 0, 0, ''));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(1,'3% Loyalty 000302992000000040', 1.14, 0));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('Ñëîéêà ñ ñàðäåëüêîé', 49.90, 1000, 0, 0, ''));

  AddLine('PrintRecItemAdjustment');
  Check(FiscalPrinter.PrintRecItemAdjustment(1,'3% Loyalty 000302992000000040', 1.50, 0));

  AddLine('PrintRecItem');
  Check(FiscalPrinter.PrintRecItem('ÒÐÊ 4:Àè-95-Ê5', 199.82, 5430, 0, 36.80, ''));
...
begin
  ...
  FiscalPrinter.FiscalReceiptType := FPTR_RT_SALES;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('****  4627087921934 ÒÓÀËÅÒÍÀß ÁÓÌÀÃÀ LU', 44.9, 1000, 1, 44.9, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  2334257000000 ÏÈÐÎÃ ÏÅ×ÅÍÛÉ Ñ ÊÀÏÓ', 66.74, 284, 2, 235, 'êã'));
  Check(FiscalPrinter.PrintRecItem('****  4607016245485 ÊÐÓÏÀ ÃÐÅ×. ßÄÐÈÖÀ 1', 77, 1000, 2, 77, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  4627116240333 ÒÂÎÐÎÆÍÈÊÈ Ñ ÌÀËÈÍÎÉ', 149, 1000, 2, 149, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  4690228004575 ÌÎËÎÊÎ ÏÈÒ. ÄÂÄ ÓËÜÒ', 71, 1000, 2, 71, 'øò'));
  Check(FiscalPrinter.PrintRecItem('**02  4607012937421 ÌÎËÎÊÎ ×ÓÄÎ ØÎÊÎËÀÄ', 79.9, 1000, 2, 79.9, 'øò'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, ' ckugka', 19.98, 2));
  Check(FiscalPrinter.PrintRecItem('****            322 ÁÀÍÀÍÛ', 66.45, 970, 1, 68.5, 'êã'));
  Check(FiscalPrinter.PrintRecItem('****  4605246006289 ÊÎÔÅ ÆÀÐÄÈÍ ÊÅÍÈß 1/', 249.9, 1000, 1, 249.9, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  4650001683571 ÂÀÒÍÛÅ ÏÀËÎ×ÊÈ 100ØÒ', 18.9, 1000, 1, 18.9, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  4650001683571 ÂÀÒÍÛÅ ÏÀËÎ×ÊÈ 100ØÒ', 18.9, 1000, 1, 18.9, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  2413343000008 ÕËÅÁ ÑÅÐÃÅÅÂÑÊÈÉ 1/5', 27, 1000, 2, 27, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  2411271000008 ÐÎËË ÔÈËÀÄÅËÜÔÈß Ñ Î', 194, 1000, 2, 194, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****             82 ÄÎÌÀØÍßß ÊÓÕÍß', 277.86, 753, 1, 369, 'êã'));
  Check(FiscalPrinter.PrintRecItem('****  4601713006240 ÒÓËÜÑÊÈÉ ÏÐßÍÈÊ Ñ ÂÀ', 27.4, 1000, 1, 27.4, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  4601713006240 ÒÓËÜÑÊÈÉ ÏÐßÍÈÊ Ñ ÂÀ', 27.4, 1000, 1, 27.4, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  4607068628410 ÆÈÄÊÎÅ ÌÛËÎ AURA ÀÍÒ', 50, 1000, 1, 50, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  7622210459299 ÏÅ×ÅÍÜÅ ÞÁÈËÅÉÍÎÅ ÊÀ', 38.5, 1000, 1, 38.5, 'øò'));
  Check(FiscalPrinter.PrintRecItem('****  7622210453853 ÏÅ×ÅÍÜÅ ÞÁÈËÅÉÍÎÅ  Ì', 38.5, 1000, 1, 38.5, 'øò'));
  Check(FiscalPrinter.PrintRecSubtotal(1503.37));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'ÌÀÐÊÈ', 1503.37));
  Check(FiscalPrinter.PrintRecTotal(100, 100, '0'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  ...
  FiscalPrinter.FiscalReceiptType := FPTR_RT_SALES;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('ÊÓÐÀÃÀ', 71.6, 180, 1, 399.2, 'êã'));
  Check(FiscalPrinter.PrintRecTotal(71.6, 71.6, '1'));
  Check(FiscalPrinter.PrintRecMessage(''));
  Check(FiscalPrinter.PrintRecMessage('                                                '));
  Check(FiscalPrinter.PrintRecMessage(' ****************************************       '));
  Check(FiscalPrinter.PrintRecMessage('           Óâàæàåìûé ïîêóïàòåëü!                '));
  Check(FiscalPrinter.PrintRecMessage('      Íà òîâàðû, ó÷àñòâóþùèå â àêöèè,           '));
  Check(FiscalPrinter.PrintRecMessage('          ñêèäêà íà ñóììó ïîêóïêè               '));
  Check(FiscalPrinter.PrintRecMessage('           íå ðàñïðîñòðàíÿåòñÿ!                 '));
  Check(FiscalPrinter.PrintRecMessage('     ****** ÑÏÀÑÈÁÎ ÇÀ ÏÎÊÓÏÊÓ ******           '));
...
begin
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('1: 3299852 Ïðîäóêò ÀËÜÏÈÉÑÊÀß ÊÎÐÎÂÊÀ 50', 0, 1000, 0, 45.99, 'øò'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, '', 0.08, 0));
  Check(FiscalPrinter.PrintRecItem('2: 3020921 Îãóðöû ñðåäíåïëîäíûå 1êã     ', 0, 1226, 0, 75.5, 'êã'));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, '', 0.17, 0));
  Check(FiscalPrinter.PrintRecItem('3: 3255319 Êàïóñòà ÊÈÒÀÉÑÊÀß 1êã        ', 0, 1272, 0, 127.36, 'êã'));
  ...
  pData := 1;
  pString := #$0D#$0A'Ïîëó÷è ñêèäêó 15% íà âñþ ïðîäóêöèþ ÏÐÎÑÒÎÊÂÀØÈÍÎ ñ 23 èþíÿ ïî 29 èþíÿ';
...
begin
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Ïîí÷èê ñ  êàêàî ãëàç', 49.9, 1000, 0, 49.9, ''));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêà çà õîðîøåå ïîâåäåíèå', 29.94, 0));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêà çà õîðîøåå ïîâåäåíèå', 10, 0));
  Check(FiscalPrinter.PrintRecItem('Donut ñ áåëîé ãëàçóð', 49.9, 1000, 0, 49.9, ''));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, 'Ñêèäêà çà õîðîøåå ïîâåäåíèå', 29.94, 0));
  Check(FiscalPrinter.PrintRecSubtotal(0));
...
begin
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('ËÈÌÎÍÀÄ ÑÀÐÎÂÀ ËÈÌÎÍ', 57, 1000, 1, 57, 'øò'));
  Check(FiscalPrinter.PrintRecItem('ÒÂÎÐÎÃ 12% 200Ã', 79.8, 2000, 2, 39.9, 'øò'));
  Check(FiscalPrinter.PrintRecSubtotal(136.8));
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'Îêðóãë.ñäà÷è', 0.3));
...
begin
  ...
  Check(FiscalPrinter.PrintRecRefund('ÒÐÊ 1:Àè-92-Ê4              Òðç23652', 199.2, 0));
  FiscalPrinter.PreLine := '                                    2,00 X 49,90';
  Check(FiscalPrinter.PrintRecRefund('Áàòîí÷èê Ïèêíèê Ãðåö', 99.80, 0));
  FiscalPrinter.PreLine := '                                    4,00 X 39,90';
  Check(FiscalPrinter.PrintRecRefund('Ïå÷.Òâèêñ ìîë.øîê.55', 159.60, 0));
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(459.32, 459.32, '0'));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:       8200'));
  Check(FiscalPrinter.PrintRecMessage('ID ïðîäàæè: 8199 (459,32 ðóá)'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  ...
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  FiscalPrinter.PreLine := '                                    5,88 X 34,00';
  Check(FiscalPrinter.PrintRecItem('ÒÐÊ 1:Àè-92-Ê4              Òðç23652', 199.2, 0, 0, 0, ''));
  FiscalPrinter.PreLine := '                                    2,00 X 49,90';
  Check(FiscalPrinter.PrintRecItem('Áàòîí÷èê Ïèêíèê Ãðåö', 99.80, 0, 0, 0, ''));
  FiscalPrinter.PreLine := '                                    4,00 X 39,90';
  Check(FiscalPrinter.PrintRecItem('Ïå÷.Òâèêñ ìîë.øîê.55', 159.60, 0, 0, 0, ''));
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(459.32, 459.32, '0'));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:       8200'));
  Check(FiscalPrinter.PrintRecMessage('ID ïðîäàæè: 8199 (459,32 ðóá)'));
...
begin
  ...
  Check(FiscalPrinter.PrintRecSubtotalAdjustment(FPTR_AT_AMOUNT_DISCOUNT, 'Ñêèäêà 1', 0.99));
...
begin
  FiscalPrinter.FiscalReceiptType := FPTR_RT_SALES;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Coca-cola ïë.áóò. 1ë', 99, 1000, 0, 99, ''));
  ...
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:    3946640'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  ...
  Check(FiscalPrinter.PrintRecRefund('ÒÐÊ 3:Àè-95-Ê5               Òðç1390', 499.74, 0));
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(499.74, 499.74, '0'));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:    3944628'));
  Check(FiscalPrinter.PrintRecMessage('ID ïðîäàæè: 3944627 (499.74 ðóá)'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  Check(FiscalPrinter.ResetPrinter);
  Check(FiscalPrinter.BeginNonFiscal);
  Check(FiscalPrinter.PrintNormal(2, 'Çàëîãîâàÿ êâèòàíöèÿ'));
  Check(FiscalPrinter.PrintNormal(2, 'ÒÐÊ  4   : Àè-95-Ê5           '));
  Check(FiscalPrinter.PrintNormal(2, 'Ïîëó÷åíî : 23.5 liter x 36.80ð. = 864.80ð.'));
...
begin
  ...
  Check(FiscalPrinter.PrintRecItem('Ïîïîëíåíèå áàíêîâñêî', 100, 1000, 0, 0, ''));
  Check(FiscalPrinter.PrintRecItem('Ãàç áàëëîí 5êã', 100, 1000, 0, 0, ''));
  Check(FiscalPrinter.PrintRecItem('Ñàõàð ïàêåòèê 5 ãð', 200, 1000, 0, 0, ''));
  Check(FiscalPrinter.PrintRecItem('×àé 120 ìë àâòîìàò', 300, 1000, 0, 0, ''));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: Ðûæèêîâà Àí. ID: 273657254'));

  FiscalPrinter.PostLine := 'Áàíê.Êàðòà        N#XXXXXX4058                                                =55.00';
...
begin
  ...
  Check(FiscalPrinter.PrintNormal(2, 'Íîìåð êàññû: 1'));
  Check(FiscalPrinter.PrintNormal(2, 'Îïåðàòîð:ts'));
  Check(FiscalPrinter.PrintNormal(2, 'Íîìåð òðàíçàêöèè:     22377'));
  Check(FiscalPrinter.PrintNormal(2, '------------------------------------'));
  Check(FiscalPrinter.PrintNormal(2, 'Îòìåíà òðàíçàêöèè'));
  Check(FiscalPrinter.PrintNormal(2, 'Óòâåðäèë:  ts'));
  Check(FiscalPrinter.PrintNormal(2, 'Ïðè÷èíà:  Håïðàâèëüíàÿ ÒÐÊ'));
  Check(FiscalPrinter.PrintNormal(2, ''));
  Check(FiscalPrinter.PrintNormal(2, 'Êîë-âî Íàçâàíèå Öåíà  Îïåðàöèÿ'));
  Check(FiscalPrinter.PrintNormal(2, ''));
  Check(FiscalPrinter.PrintNormal(2, '1 Àâòîìîáèëüíûé áåíçèí Ïóëüñàð ÀÈ-92-5       0.000    Îòìåíà'));
  Check(FiscalPrinter.PrintNormal(2, 'Áàíê.Êàðòà        N#XXXXXX4058                                                =55.00'));
  Check(FiscalPrinter.EndNonFiscal);
end;
...
begin
  ...
  Check(FiscalPrinter.PrintRecRefund('ÒÐÊ 3:ÀÈ-92', 1472, 0));
  Check(FiscalPrinter.PrintRecSubtotal(1472));
  Check(FiscalPrinter.PrintRecTotal(1472, 1472, '0'));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:    3945836'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Coca-cola ïë.áóò. 1ë', 297, 3000, 0, 99, ''));
  FiscalPrinter.PostLine := '  17.480    *36.80';
  Check(FiscalPrinter.PrintRecItem('ÒÐÊ 4:Àè-95-Ê5               Òðç1818', 643.26, 1000, 0, 643.26, ''));
  FiscalPrinter.PostLine := 'Áàíê.Êàðòà        N#XXXXXX4058             =640.26';
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(940.26, 640.26, '1'));
  Check(FiscalPrinter.PrintRecTotal(940.26, 300, '0'));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:    3946653'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Coca-cola ïë. 0.5ë', 75, 1000, 0, 75, ''));
  ...
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: ts ID:    3946670'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  FiscalPrinter.FiscalReceiptType := FPTR_RT_SALES;
  Check(FiscalPrinter.BeginFiscalReceipt(False));
  Check(FiscalPrinter.PrintRecItem('Øîêîëàä Ðîññèéñêèé ò', 95, 1000, 0, 95, ''));
  Check(FiscalPrinter.PrintRecItemAdjustment(1, '5% Loyalty  000302000400292392', 4.75, 0));
  FiscalPrinter.PostLine := 'Áàíê.Êàðòà        N#XXXXXX0489      ';
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(90.25, 90.25, '1'));
  Check(FiscalPrinter.PrintRecMessage('Îïåðàòîð: Áóð÷åíêîâà ID:     295117'));
  Check(FiscalPrinter.EndFiscalReceipt(True));
end;
...
begin
  ...
  Check(FiscalPrinter.PrintRecItem('ÀÈ-92-3in', 0, 2610, 4, 0, ''));
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(0, 0, '0'));
...
begin
  ...
  FiscalPrinter.FiscalReceiptType := FPTR_RT_SALES;
  Check(FiscalPrinter.BeginFiscalReceipt(True));
  Check(FiscalPrinter.PrintRecItemRefund('ÀÈ-92-3in', 0, 2610, 4, 0, ''));
  Check(FiscalPrinter.PrintRecSubtotal(0));
  Check(FiscalPrinter.PrintRecTotal(0, 0, '0'));
  Check(FiscalPrinter.EndFiscalReceipt(False));
end;
...
begin
  ...
  FiscalPrinter.FiscalReceiptType := FPTR_RT_SALES;
  Check(FiscalPrinter.BeginFiscalReceipt(True));
  check(FiscalPrinter.PrintRecItem('1:95 Îëèâêè ITLV çåëåíûå 300ã', 124, 1000, 1, 124, 'øò'));
  check(FiscalPrinter.PrintRecItem('2:29665 Ìîëîêî ÏÈÑÊÀÐÅÂÑÊÎÅ 1ë', 53.99, 1000, 2, 53.99, 'øò'));
  check(FiscalPrinter.PrintRecItem('3:3246577 Îãóðöû êîðîòêîïëîäíûå 1êã', 67.8, 1384, 2, 48.99, 'êã'));
  check(FiscalPrinter.PrintRecItem('4*3248766 ßáëîêè ÃËÎÑÒÅÐ 1êã', 166.19, 2080, 1, 79.9, 'êã'));
  check(FiscalPrinter.PrintRecItem('5*3255455 Ïåðåö ÄÎËÌÀ 1êã', 23.82, 136, 2, 175.18, 'êã'));
  check(FiscalPrinter.PrintRecItem('6:2050923 Ñðåäñòâî ÑÀÍÎÊÑ ÷èñòÿùåå 750ã', 44.59, 1000, 1, 44.59, 'øò'));
  check(FiscalPrinter.PrintRecItem('7*3501031 Ñðåäñòâî FAIRY 450ìë', 49.9, 1000, 1, 49.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('8*2038357 Çóá.ïàñòà COLGATE 100ìë', 58.8, 1000, 1, 58.8, 'øò'));
  check(FiscalPrinter.PrintRecItem('9:3631647 Äèñêè ÊÐÓÃËÛÉ ÃÎÄ 80øò', 19.39, 1000, 1, 19.39, 'øò'));
  check(FiscalPrinter.PrintRecItem('10*3620583 Èçä.ìàê.AIDA 450ã', 29.9, 1000, 2, 29.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('11*3448338 ×àé LIPTON ÷åðíûé 100õ2ã', 159, 1000, 1, 159, 'øò'));
  check(FiscalPrinter.PrintRecItem('12*3500750 Ñðåäñòâî FAIRY 450ìë', 49.9, 1000, 1, 49.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('13*3620583 Èçä.ìàê.AIDA 450ã', 29.9, 1000, 2, 29.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('14*3620583 Èçä.ìàê.AIDA 450ã', 29.9, 1000, 2, 29.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('15:3420638 Äåçîäîðàíò LADY êàðàíäàø 45ã', 144, 1000, 3, 144, 'øò'));
  check(FiscalPrinter.PrintRecItem('16*3618920 ÑÌÑ LOSK ÑÅÍÑÈÒÈÂ 4,5êã', 299, 1000, 1, 299, 'øò'));
  check(FiscalPrinter.PrintRecItem('17*18898 Ãðåéïôðóò îòáîðíûé 1êã', 147.62, 1642, 1, 89.9, 'êã'));
  check(FiscalPrinter.PrintRecItem('18:3231971 Òîìàòû 1êã', 239.34, 2522, 2, 94.9, 'êã'));
  check(FiscalPrinter.PrintRecItem('19*3757 Áàíàíû 1êã', 113.17, 2268, 1, 49.9, 'êã'));
  check(FiscalPrinter.PrintRecItem('20*88 Ìàñëèíû ITLV ÑÓÏÅÐ ÷åðíûå 350ã', 99.9, 1000, 1, 99.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('21:18077 Ïåòðóøêà ïàêåò 100ã', 24.9, 1000, 2, 24.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('22:18074 Óêðîï ïàêåò 100ã', 24.9, 1000, 2, 24.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('23*88 Ìàñëèíû ITLV ÑÓÏÅÐ ÷åðíûå 350ã', 99.9, 1000, 1, 99.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('24*3445023 Àâîêàäî 1øò', 29.9, 1000, 1, 29.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('25*3445023 Àâîêàäî 1øò', 29.9, 1000, 1, 29.9, 'øò'));
  check(FiscalPrinter.PrintRecItem('26:3300463 Ïàêåò ÊÀÐÓÑÅËÜ 40õ65', 6.04, 1000, 1, 6.04, 'øò'));
  check(FiscalPrinter.PrintRecItem('27:3300463 Ïàêåò ÊÀÐÓÑÅËÜ 40õ65', 6.04, 1000, 1, 6.04, 'øò'));
  check(FiscalPrinter.PrintRecItem('28:3300463 Ïàêåò ÊÀÐÓÑÅËÜ 40õ65', 6.04, 1000, 1, 6.04, 'øò'));
  check(FiscalPrinter.PrintRecSubtotal(2177.73));
  check(FiscalPrinter.PrintRecSubtotalAdjustment(1, 'ÎÊÐÓÃËÅÍÈÅ', 0.73));
